...
class Medoo
{
	...
	public function __construct(array $options) {
		...
		if (isset($options[ 'tablepre' ])) {
			$this->prefix = $options[ 'tablepre' ];
		}
		...
		if (isset($options[ 'pdo' ])) {
			if (!$options[ 'pdo' ] instanceof PDO) {
				\PHF\Exception::throw('Invalid PDO object supplied',100);
			}
		...
		if (isset($options[ 'dsn' ])) {
			if (is_array($options[ 'dsn' ]) && isset($options[ 'dsn' ][ 'driver' ])) {
                ...
                \PHF\Exception::throw('Invalid DSN option supplied',100);
			}
		} else {
			...
			switch ($this->type) {
				...
				case 'oracle':
					$attr = [
						'driver' => 'oci',
						'dbname' => $options[ 'host' ] ?
							'//' . $options[ 'host' ] . ($is_port ? ':' . $port : ':1521') . '/' . $options[ 'base' ] :
							$options[ 'base' ]
					];
				...
				case 'mssql':
					if (isset($options[ 'driver' ]) && $options[ 'driver' ] === 'dblib') {
						$attr = [
							'driver' => 'dblib',
							'host' => $options[ 'host' ] . ($is_port ? ':' . $port : ''),
							'dbname' => $options[ 'base' ]
						];
						...
						$attr = [
							'driver' => 'sqlsrv',
							'Server' => $options[ 'host' ] . ($is_port ? ',' . $port : ''),
		...
		if (!isset($attr)) {
            \PHF\Exception::throw('Incorrect connection options',100);
		}

		$driver = $attr[ 'driver' ];

		if (!in_array($driver, PDO::getAvailableDrivers())) {
            \PHF\Exception::throw("Unsupported PDO driver: {$driver}",100);
		}
		...
		try {
			$this->pdo = new PDO(
				...
				isset($options[ 'pwd' ]) ? $options[ 'pwd' ] : null,
				$option
			);
            ...
            \PHF\Exception::throw($e->getMessage(),100);
		}
	}
